...
def adduser(request):
	if request.method=='GET':
		...
		# 文件上传
		if not myfile:
			return HttpResponse("<script>alert('请选择头像');history.back(-1)</script>")
		head_url=upload(myfile)
		...
		try:
			...
			return HttpResponse('<script>alert("添加成功");location.href="'+reverse('myadmin_adduser')+'"</script>')
...
def uuload(myfile):
	# 新名字
	filename = str(time.time())+"."+myfile.name.split('.').pop()
	destination = open("./static/media/article/"+filename,"wb+")
	for chunk in myfile.chunks():		# 分块写入文件  
		destination.write(chunk)
	destination.close()
	# /static/pics/filename
	return '/static/media/article/'+filename